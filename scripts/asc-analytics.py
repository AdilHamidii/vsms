...
def rows(label):
    """Every data row of one report. Segments are concatenated, so the header repeats."""
...
def cmd_daily():
    ...
    for day in sorted(d):
        ...
        print(f"{day:12}{c['impr_search']:>12}{c['impr_other']:>11}{pv:>11}{c['new']:>13}"
              f"{(pv / impr * 100 if impr else 0):>8.1f}%{(c['new'] / pv * 100 if pv else 0):>8.0f}%")
...
def cmd_territory():
    """Impressions per territory, last 4 complete days against the 8 before them."""
    ...
    days = sorted({d for c in terr.values() for d in c})
    if len(days) < 12:
        sys.exit("not enough history for a comparison")
    now, base = days[-4:], days[-12:-4]
    print(f"compare {base[0]}..{base[-1]}  ->  {now[0]}..{now[-1]}  (impressions/day)\n")
    print(f"{'terr':6}{'before':>9}{'after':>9}{'change':>9}")
    ranked = sorted(terr, key=lambda t: -window(terr[t], base[0], base[-1]))
    for t in ranked[:15]:
        b, n = window(terr[t], base[0], base[-1]), window(terr[t], now[0], now[-1])
        print(f"{t:6}{b:>9.0f}{n:>9.0f}{((n - b) / b * 100 if b else 0):>8.0f}%")
