#!/usr/bin/env python3
"""Reprice the Mail subscriptions for NEW subscribers (owner decision 2026-09-25):
mail.monthly $2.99 -> $4.99, mail.yearly $29.99 -> $39.99, in every territory.
...
price of the USA point. This is an INCREASE, so `preserveCurrentPrice` is True
— every existing subscriber keeps what they pay today and Apple sends no
price-increase consent request; only new subscriptions pay the new price.

The client renders StoreKit's `displayPrice`, so every shipped build shows the
new price the moment it takes effect — nothing in any build hardcodes it.
...
TARGETS = [
    ("6803258564", "com.anthersystems.VirtualSIM.mail.monthly", "4.99"),
    ("6803258736", "com.anthersystems.VirtualSIM.mail.yearly", "39.99"),
]
...
# earliest date Apple accepts MOVES: tomorrow on 2026-09-01, but at 17:29Z on
# 2026-09-25 it refused 09-26 with "must be on or after 2026-09-27". Hence
# `--start YYYY-MM-DD`; the refusal names the earliest date it will take.
import datetime
START_DATE = next((a.split("=", 1)[1] for a in sys.argv if a.startswith("--start=")),
...
def main():
    for sub, pid, numeral in TARGETS:
        ...
        for t in terrs:
            ...
            # A failed read must not fall through to equalization: on
            # 2026-09-25 two transient misses put ALB/ARM on $5.99, not $4.99.
            for _ in range(3):
                code, r = call("GET", f"/v1/subscriptionPricePoints/{pid_same}")
                if code in (200, 404):
                    break
                time.sleep(2)
            else:
                print(f"  FAILED: price point read for {t} kept failing ({code})"); sys.exit(1)
        ...
        for t, (pt, _) in chosen.items():
            code, r = call("POST", "/v1/subscriptionPrices", {"data": {
                ...
                "attributes": {"preserveCurrentPrice": True, "startDate": START_DATE},
                "relationships": {
