-- `/subs`: stop counting credits-billed lines against Apple subscriptions.
--
-- The divergence warning is the only place either half is checked — a live
-- line with no subscription is rent we pay for nothing, and a live
-- subscription with no line is a customer paying for nothing. It compared
-- APPLE subscriptions against ALL live lines, so the owner's own
-- credits-billed test line made it fire permanently. An alert that is always
-- on trains the reader to ignore it, which is how the next real one is missed.
--
-- Regenerated from the live `pg_get_functiondef` and edited surgically: the
-- ONLY difference is the two new payload keys below. Nothing existing was
-- removed, so every other reader of `ops_subs()` is unaffected.
...
as $ops_subs$
  ...
  -- `coalesce` â a subscriber who went through a grace period and then
  ...
  select jsonb_build_object(
    ...
    -- everything" â same rule as /orders). auto_renew is trustworthy as of
    ...
    -- (the whole entitled set is small by construction â one live line per
    ...
    -- HUNK (20260908220000): the divergence warning compares subscriptions to
    -- LINES, and it was counting credits-billed lines against APPLE
    -- subscriptions — so the owner's own credits test line read as a permanent
    -- "these must match" mismatch. A warning that is always lit is a warning
    -- nobody reads, and this one exists to catch rent we pay for nothing.
    --
    -- Split rather than filtered: a credits line that has outlived its funding
    -- is ALSO a real leak, just a different one, so it stays visible on its
    -- own line instead of being quietly dropped from the comparison.
    'lines_active_apple', (select count(*)::int from lns
                            where billing = 'apple'
                              and status in ('active','grace','past_due')),
    'lines_active_credits', (select count(*)::int from lns
                              where billing is distinct from 'apple'
                                and status in ('active','grace','past_due')),
