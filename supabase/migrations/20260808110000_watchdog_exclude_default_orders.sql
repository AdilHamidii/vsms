...
begin
  ...
  select count(*), count(*) filter (where o.otp is not null or o.status = 'received')
   ...
   where o.closed_at >= now() - interval '72 hours'
     ...
     -- Orders on the app's OWN pre-selection are excluded: the user
     -- never chose that service, so the number was never entered
     -- anywhere and no code was ever requested. Counting them as
     -- delivery failures measures our steering, not the provider.
