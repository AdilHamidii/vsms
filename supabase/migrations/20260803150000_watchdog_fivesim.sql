-- Point the watchdog at 5sim, and stop it going permanently red at cutover.
--
-- 🔴 THE BUG THIS PREVENTS. run_watchdog's route-cost check read
...
-- The moment the last route is re-homed to 5sim that predicate matches ZERO
-- rows, so max() returns NULL, so the `v_ts is null` arm fires -- forever, from
-- the instant of cutover. And a permanently-red watchdog is not just noise:
-- winback gates its stranded-credit cohort on `failing === 0`, so it would have
-- silently closed that cohort. Same bug class as the daily-credit check fixed
-- hours earlier on 2026-08-03, and the fourth instance in this codebase.
--
-- HeroSMS is NOT retired -- it now serves the temp EMAIL line on the same
-- account and the same key. Its BALANCE still matters and is still reported by
-- /balance; only its route-cost sync is gone, so only that check is replaced.
--
-- The two SMSPVA cursor checks are GATED, not deleted, on
-- app_config.smspva_retired -- so un-retiring SMSPVA restores its monitoring in
-- the same step. Compared as TEXT, never cast: run_watchdog is deliberately
-- plain SQL so it still evaluates when the whole edge layer is down, and a junk
-- value must not raise.
--
-- Regenerated from pg_get_functiondef and diffed clause by clause, as this
-- function demands after a past rewrite silently narrowed a delivery check into
-- unreachability: EXACTLY 6 HUNKS DIFFER, confined to the herosms check and the
-- two smspva gates, and all 15 checks are still present.
